...
function validateTextBox(x, y, z) {
  ...
  for (var i = 0; i < input.length; i++) {
    if (input[i].value.length != 0) {
      ...
      var regular_express;
      regular_express = /^((\s?[a-zA-Z0-9àáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ:_-]*)([(](\s?[a-zA-Z0-9àáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ:_-]+){1,10}[)])?){1,30}$/;
      var result;
      result = regular_express.test(input[i].value.toLowerCase());
      console.log(input[i].value + ": " + result);
      if (result == true) {
        count++;
        err[i].innerHTML = "";
      } else {
  ...
  console.log("validate text box: " + count);
